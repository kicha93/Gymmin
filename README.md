...
- Local emulator APKs are built without GitHub publishing through `npm run mobile:emulator:apk`; no API URL argument is needed because the wrapper uses the same backend health-check and automatic tunnel flow as the phone one-click build. The default target is standalone `x86_64`; pass `-- -Target universal` for one APK containing `armeabi-v7a`, `arm64-v8a`, `x86` and `x86_64`. Artifacts are written only to `.artifacts`.
