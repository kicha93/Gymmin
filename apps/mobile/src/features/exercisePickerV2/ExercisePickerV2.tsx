...
export function ExercisePickerV2({ disabled = false, favoriteExerciseIds, language, onChange, onToggleFavorite, placeholder, stageType, t, theme, usageById, value }: Props) {
  ...
  const filterOptions = filterSection === "muscle"
    ...
    : filterSection === "equipment"
      ...
      : exercisePickerLibraryTiers.map((key) => ({ id: key, label: key === "main" ? (language === "pl" ? "Podstawowe" : "Core") : key === "progression" ? (language === "pl" ? "Progresje" : "Progressions") : key, selected: tiers.has(key), onPress: () => toggle(setTiers, key) }));
...
const localStyles = StyleSheet.create({
  ...
  filterRow: { alignItems: "center", gap: 8, paddingHorizontal: 16, paddingVertical: 11 }, filterChip: { alignItems: "center", borderRadius: 999, borderWidth: 1, flexDirection: "row", gap: 6, minHeight: 38, paddingHorizontal: 12 }, filterChipText: { fontSize: 13, fontWeight: "700", maxWidth: 145 }, clearFilters: { justifyContent: "center", minHeight: 38, paddingHorizontal: 6 },
