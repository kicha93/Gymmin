...
8. creates an immutable GitHub Release identified by the APK SHA-256 (for example `android-v1.0-vc2-fe9fdac543ab`) in the main public `kicha93/Gymmin` source repository, publishes the versioned APK and a consistently named direct-download APK, marks that release as latest, then verifies every remote asset name, byte size, and digest. Existing assets are never replaced with `--clobber`, so a later publication cannot invalidate an in-progress download.

The permanent public entry point is `https://github.com/kicha93/Gymmin/releases/latest/download/Gymmin-arm64-v8a-release.apk`. GitHub resolves it to the immutable asset from the latest verified release.
