...
- Prompt V4 designs all requested sessions as one coordinated week using current evidence research: movement patterns remain stable for progression, while exact exercise IDs are unique between days. The review step blocks unjustified cross-workout duplicates; repeated warm-ups and one justified strength-anchor practiced across at most two days are treated separately.
